...
def build_index():
    ...
    # ensure client-side chipsquirt JS is present (unit=None → teal family)
    html2 = INDEX_HTML.read_text()
    updated2 = _ensure_chipsquirt_js(html2)
    if updated2 != html2:
        INDEX_HTML.write_text(updated2)
        print("  index: injected chipsquirt JS")
...
def _apply_chipsquirt(html: str, unit: str) -> str:
    ...
    html = chipsquirt.apply_to_headers(html, family, levels=['h1'], mode='fourth-power-interval')
    html = chipsquirt.apply_to_headers(html, family, levels=['h2', 'h3'], mode='orbit')
    return html


# Client-side chipsquirt JS injected into index pages (unit and main).
# Applied after DOM load so the build pipeline's regex matchers still see clean tags.
_CHIPSQUIRT_JS = """
  <script>
  /* chipsquirt-js */
  (function(){
    var FAMILIES={teal:{hc:172,hd:12,sat:70,lc:28,lr:8},slate:{hc:215,hd:8,sat:40,lc:28,lr:8},forest:{hc:140,hd:10,sat:60,lc:25,lr:8},amber:{hc:36,hd:8,sat:75,lc:28,lr:8},violet:{hc:268,hd:14,sat:60,lc:26,lr:8},magenta:{hc:312,hd:10,sat:65,lc:26,lr:8}};
    var UNIT_FAMILIES={'01':'teal','02':'slate','03':'forest','04':'amber','05':'violet','06':'magenta','07':'teal','08':'slate','09':'amber','10':'forest','11':'violet'};
    function hsvToRgb(h,s,v){h=((h%360)+360)%360;var c=v*s,x=c*(1-Math.abs((h/60)%2-1)),m=v-c,r,g,b;if(h<60){r=c;g=x;b=0;}else if(h<120){r=x;g=c;b=0;}else if(h<180){r=0;g=c;b=x;}else if(h<240){r=0;g=x;b=c;}else if(h<300){r=x;g=0;b=c;}else{r=c;g=0;b=x;}return[Math.round((r+m)*255),Math.round((g+m)*255),Math.round((b+m)*255)];}
    function hslToRgb01(h,s,l){s/=100;l/=100;h=((h%360)+360)%360;var c=(1-Math.abs(2*l-1))*s,x=c*(1-Math.abs((h/60)%2-1)),m=l-c/2,r,g,b;if(h<60){r=c;g=x;b=0;}else if(h<120){r=x;g=c;b=0;}else if(h<180){r=0;g=c;b=x;}else if(h<240){r=0;g=x;b=c;}else if(h<300){r=x;g=0;b=c;}else{r=c;g=0;b=x;}return[r+m,g+m,b+m];}
    function letterColor(i,fam,mode,phase){var hc=fam.hc,hd=fam.hd,sat=fam.sat,lc=fam.lc,lr=fam.lr,freq=0.45,PHI=1.6180339887,PHI2=2.6180339887;
      if(mode==='orbit'){var h_hsv=((hc+hd*Math.sin(i*freq+phase))%360+360)%360,s_hsv=Math.max(0,Math.min(1,sat/100+0.18*Math.sin(i*freq*PHI+phase+1.1))),v_hsv=Math.max(0.08,Math.min(0.60,0.30+0.18*Math.sin(i*freq*PHI2+phase+2.3))),rgb=hsvToRgb(h_hsv,s_hsv,v_hsv);return'rgb('+rgb+')';}
    ...
    document.addEventListener('DOMContentLoaded',function(){
      var unit=(window.location.pathname.match(/\\/([0-9]{2})\\//)||[])[1]||null,famName=UNIT_FAMILIES[unit]||'teal',fam=FAMILIES[famName];
      // h1 only — section headers (h2) stay plain
      document.querySelectorAll('h1').forEach(function(el){colorizeEl(el,fam,'fourth-power-interval');});
      // syllabus: unit titles cycle through all families
      document.querySelectorAll('.unit-title').forEach(function(el,idx){colorizeEl(el,FAMILIES[FAMILY_CYCLE[idx%6]],'orbit');});
      // unit pages: bold item names in materials, links in repos and documents
      document.querySelectorAll('.materials li strong').forEach(function(el){colorizeEl(el,fam,'orbit');});
      document.querySelectorAll('.repos li a').forEach(function(el){colorizeEl(el,fam,'orbit');});
      document.querySelectorAll('.documents li a:first-child').forEach(function(el){colorizeEl(el,fam,'orbit');});
    });
  })();
  </script>
"""
...
def build_unit_page(unit: str) -> bool:
    ...
    # ensure client-side chipsquirt JS is present
    html3 = unit_html.read_text()
    updated3 = _ensure_chipsquirt_js(html3)
    if updated3 != html3:
        unit_html.write_text(updated3)
        print(f"  unit {unit}: injected chipsquirt JS", flush=True)

    if not unit_changed and essay_count == 0 and updated2 == html2 and updated3 == html3:
